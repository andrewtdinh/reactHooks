...
export default class Calculator extends React.Component {
  ...
  componentDidMount = () => {
    fetch(
      "http://api.apixu.com/v1/current.json?key=478f53ca816141a79a8102217191906&q=Mohali"
    )
      .then(res => res.json())
      .then(
        result => {
  ...
  render() {
    const { scale, value } = this.state;
    const celsius =
      scale === "Fahrenheit" ? this.tryConvert(value, this.toCelsius) : value;
    const fahrenheit =
      scale === "Celsius" ? this.tryConvert(value, this.toFahrenheit) : value;
    return (
